...
function OperatingTimes({openHours, setOpenHours,addSecondTimeSlot, setAddSecondTimeSlot}: any) {
...
function handleInputChange(dayIndex:number, timeIndex:number, e:ChangeEvent) {
...
function handleCheckboxChange(index:number, e:ChangeEvent) {
  ...
  if (!checked) {
    newOpenHours[index].timePeriods = [      { openTime: '', closingTime: '' },      { openTime: '', closingTime: '' },    ];
  }
  setOpenHours(newOpenHours);
}
  ...
  return (
    <div className='bg-myBlue/50 p-1 rounded-lg mt-2'>
            <button type="button" onClick={() => setAddSecondTimeSlot(!addSecondTimeSlot)}>
      ...
      {openHours.map((day: Day, index:number) => (
        <div key={day.day} className=" items-center  ">
           <div className=' flex items-center'> 
                ...
                />
                <label className='text-2xl mr-1'>
                {showDayOfWeek(day.day)}
                 </label>
